package by.epam.java.training.assanoooovi4k.lifehacks.weblayer.filter;
...
@WebFilter( urlPatterns = { "/pages/user/*" },
        initParams = { @WebInitParam(name = Constant.INDEX, value = PagePath.AUTHORIZATION_PAGE_FOR_FILTER) })
public class UserSecurityFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (httpSession.getAttribute(Constant.ROLE).equals(UserRole.GUEST)) {
